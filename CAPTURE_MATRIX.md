...
| Checkpoints | browser state + native cookie + bounded viewport screenshot | before/after action, mutating network completion, navigation, JS errors, recording boundaries; cookies/storage/DOM summary | raw `checkpoint` event + checkpoint artifacts | `checkpoints/index.json`, `checkpoint-diffs.json`, `checkpoints/<id>/...` | максимум 40 checkpoints и 24 screenshots; state bounded, diff является derivative evidence |
| JS causality | captured fetch/XHR initiator stack + forensic correlation | initiator stack, related action, request and mutation IDs | derivative analysis | `js-causality.json` | initiator stack is observed; temporal links remain explicitly inferred |
