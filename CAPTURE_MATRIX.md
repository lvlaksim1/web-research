...
| Checkpoints | browser state + native cookie + bounded viewport screenshot | before/after action, mutating network completion, navigation, JS errors, recording boundaries; cookies/storage/DOM/runtime UI | raw `checkpoint` event + checkpoint artifacts | `checkpoints/index.json`, `checkpoint-diffs.json`, `checkpoints/<id>/...` | лимиты сбрасываются на старте ZIP: максимум 80 checkpoints и 80 screenshots на recording window; state bounded, diff является derivative evidence |
...
| Frames | DOM inventory + same-origin access | src/name/id/sandbox/allow/loading, rect, sameOrigin; bounded snapshot same-origin frame | checkpoint/full snapshot | checkpoint state + page snapshot + frame diff | cross-origin содержимое недоступно по Same-Origin Policy; snapshot depth = 1 |
