...
internal object WebResearchScripts {
    fun instrumentation(): String = """
          (function(){
            ...
            const cpSelection=(d,w)=>{let o={};try{let s=w.getSelection?.(),node=n=>n?(n.nodeType===1?n:n.parentElement):null;if(s){let a=node(s.anchorNode),f=node(s.focusNode);o={type:s.type||'',collapsed:!!s.isCollapsed,anchorOffset:s.anchorOffset||0,focusOffset:s.focusOffset||0,text:String(s.toString()||'').slice(0,1000),anchorNode:a?cpKey(a,-1):'',focusNode:f?cpKey(f,-1):''}}}catch(e){o.error=String(e)}return o};
