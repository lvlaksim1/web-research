...
internal object WebResearchScripts {
    fun instrumentation(): String = """
          (function(){
            ...
            const cpShadow=(d,depth=0)=>{let roots=[],queue=[],rootLimit=depth?10:40,scanLimit=depth?1500:5000,elementLimit=depth?40:80,htmlLimit=depth?5000:10000;try{queue.push(...Array.from(d.querySelectorAll('*')))}catch(e){};let scanned=0;while(queue.length&&roots.length<rootLimit&&scanned<scanLimit){let host=queue.shift();scanned++;let root=null;try{root=host.shadowRoot}catch(e){};if(!root)continue;let all=[];try{all=Array.from(root.querySelectorAll('a,button,input,select,textarea,option,form,[role],[onclick]'))}catch(e){};let els=all.slice(0,elementLimit).map((e,i)=>cpElement(e,i,root)),fullHtml='';try{fullHtml=String(root.innerHTML||'')}catch(e){};roots.push({host:cpElement(host,roots.length,d),mode:root.mode||'open',focus:cpFocus(root),html:fullHtml.slice(0,htmlLimit),htmlTruncated:fullHtml.length>htmlLimit,elements:els,elementsTotal:all.length,elementsTruncated:all.length>els.length});try{queue.push(...Array.from(root.querySelectorAll('*')))}catch(e){}}return{captured:roots.length,truncated:queue.length>0||scanned>=scanLimit,roots:roots}};
