...
internal object WebResearchScripts {
    fun instrumentation(): String = """
          (function(){
            ...
            ['click','change','submit'].forEach(type=>document.addEventListener(type,e=>{let info=target(e.target),token='actionctx-'+wrPageId+'-'+String(++wrActionSeq).padStart(8,'0'),ctx={token:token,action:type,target:info,time:Date.now()};window.__WR_ACTIVE_ACTION_CONTEXT=ctx;send({source:'user-action',time:ctx.time,action:type,page:location.href,target:info,browserActionToken:token,eventPhase:e.eventPhase||0});window.__WR_CAPTURE_CHECKPOINT('before-action',{action:type,target:info});setTimeout(()=>{if(window.__WR_ACTIVE_ACTION_CONTEXT===ctx)window.__WR_ACTIVE_ACTION_CONTEXT=null},0);setTimeout(()=>window.__WR_CAPTURE_CHECKPOINT('after-action',{action:type,target:info}),350)},true));
